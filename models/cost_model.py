...
class CostHeader(models.Model):
    ...
    title = fields.Char(string=_("Title"))
    ...
    material_cost_markup = fields.Float(compute='_markup',
                                        string=_('Total Unit Price'))  # Material cost including markup
    ...
    # Sobreescribimos la funcion copy, para copiar el detalle de la ficha
    @api.returns('self', lambda value: value.id)
    def copy(self, cr, uid, id, default=None, context=None):
        """
        Copia de los objetos enlazados al copiar el escandallo.
        """
        _logger.debug("Default copy: ".format(default))
        cost_obj = self.browse(cr, uid, [id], context=context)
        res = models.Model.copy(self, cr, uid, id, default=default, context=context)

        vals1 = []
        for i in cost_obj.cost_ids:
            vals1.append([0, 0, {
                'cost_line_id': res,
                'product_id': i.product_id.id,
                'component_name': i.component_name,
                'unidades': i.unidades.id,
                'qty_ldm': i.qty_ldm,
        ...
        vals2 = []
        for w in cost_obj.work_ids:
            vals2.append([0, 0, {
                'cost_line_id': res,
                'product_id': w.product_id.id,
                'component_name': w.component_name,
                'unidades': w.unidades.id,
                'qty_ldm': w.qty_ldm,
                'price_unit': w.price_unit,
                'discount': w.discount,
                'price_unit_net': w.price_unit_net,
                'price_total': w.price_total,
            }])

        cost_obj1 = self.browse(cr, uid, [res], context=context)
        cost_obj1.cost_ids = vals1
        cost_obj1.work_ids = vals2
